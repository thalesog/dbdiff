...
class DbDiff {
  ...
  _compareSequences(db1, db2) {
    ...
    diff2.forEach((sequenceName) => {
      var sequence = db2.sequences.find(
    ...
    inter.forEach((sequenceName) => {
      var sequence1 = db1.sequences.find(
        (sequence) => sequenceName === this._fullName(sequence)
      );
      var sequence2 = db2.sequences.find(
        (sequence) => sequenceName === this._fullName(sequence)
      );
    ...
    table2.constraints.forEach((constraint2) => {
      ...
      if (!constraint1) {
        ...
        if (constraint2.type === 'primary') {
          ...
          var foreignKeys = constraint2.referenced_columns
            .map((s) => `${this._quote(s)}`)
            .join(', ');
          func(
      ...
      if (!t) {
        var columns = table.columns.map((col) => {
          ...
          if (col.extra === 'auto_increment') {
            ...
            var constraint = table.constraints.find(
